...
def admin_ui(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print('________________ shop uname ___________________',s_un)
            context={
                'shops' : shops,
                'users': users,
                'orders': orders,
            ...
            print('________________ shop uname ___________________',context['s_un'])
            for shop in shops:
...
def admin_del_p(request,s_un,ok):
    print('__________________Inside delete function _______________________')
...
def add_to_cart(request,ck):
    if request.method == 'POST':
        ...
        print('___________________ Post method ________________________________')
        if form.is_valid():
            print('___________________ Validation method ________________________________')
            print('_____________________ Cart in form :',request.POST['p_deliver'])
            form.save()
    products=Product.objects.all()
...
def conf_ord(request,ck):
    ...
    AddOrderSet= inlineformset_factory(Client,Orders, fields=('p_name','p_deliver','p_cost'),extra=0)
